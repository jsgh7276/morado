...
* 중요한 두 가지
  ...
  * 민감정보 관리
* 무중단 업데이트 하려면
  * 환경변수 활용할 수 없다. 파드 교체가 필요하기 때문
  * 볼륨 마운트를 활용해야 한다. 컨피그맵/시크릿 업데이트를 통해
  * 대안: 버전을 올린 새로운 설정 객체 배치 후 애플리케이션이 신규 설정을 가리키도록 하기
    * 파드가 교체되지만, 설정 변경 이력이 남고 되돌릴 수 있다.
* 민감정보 관리
  * 버전 관리하여 배포하기
  * 빈칸을 포함한 yaml 파일을 형상관리 하다가, 배치 절차 중 해당 부분을 key vault 등에서 가져온 민감 정보로 채우기
* 리소스 정리 팁
  * 리소스 yaml 파일에 정의된 리소스를 삭제할 수 있다.
  * `k delete -f todo-list/`와 같은 방식

## 6. 연습 문제
* DB 설정하기 예제
* DB 접속
  * 서버는 service의 정보로 도메인 및 포트를 알아낸다
  * volume mount된 secret의 값을 읽어서 비밀번호를 알아낸다
* 모든 주어진 yaml 파일을 배치한다.
* 그 다음 각 yaml 파일을 수정하고 배포
  * 시크릿값 수정(패스워드) 및 추가(디폴트 서버)
  * 컨피그맵 yaml 새로 만들고 ADMINER_DESIGN 값 정의
  * adminer.yaml 디플로이먼트에서 환경변수를 적절한 곳에서 가져오도록 설정
